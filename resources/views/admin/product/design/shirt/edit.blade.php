...
<div class="content-wrapper">
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li class="active">Edit Shirt Design</li>
        </ol>
    </section>
                            ...
                            <div class="tab-pane active" id="mainPane">
                                ...
                                <div class="gb-body">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name" maxlength="255" value="{{ old('name', $design->name) }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="attributeSet" class="form-control custom-select mt-15 @error('attributeSet') is-invalid @enderror" name="attributeSet">
                                            ...
                                            @foreach($attributeSets as $set)
                                                <option value="{{ $set->id }}" @if($set->id == $design->product_attribute_set_id) selected @endif> {{ $set->name }} </option>                                            
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="description" id="description" class="form-control wysiwyg @error('description') is-invalid @enderror" rows="5">{{ old('description', $design->description) }}</textarea>
                                        @error('description') <p class="error-p">{{$errors->first('description')}}</p> @enderror
                                    </div>
                                    <div class="form-group">
                                        <label for="summary">Design Summary</label>
                                        <textarea name="summary" id="summary" class="form-control @error('summary') is-invalid @enderror" rows="5">{{ old('summary', $design->summary) }}</textarea>
                                        @error('summary') <p class="error-p">{{$errors->first('summary')}}</p> @enderror
                                    </div>
                                    <div class="section-sub-title">Design Display</div>
                                    <div class="form-group">
                                        ...
                                        <select id="featured" class="form-control custom-select mt-15 @error('featured') is-invalid @enderror" name="featured">
                                            <option value="1" @if($design->featured == 1) selected @endif>Yes</option>
                                            <option value="0" @if($design->featured == 0) selected @endif>No</option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="menu" class="form-control custom-select mt-15 @error('menu') is-invalid @enderror" name="menu">
                                            <option value="1" @if($design->menu == 1) selected @endif>Yes</option>
                                            <option value="0" @if($design->menu == 0) selected @endif>No</option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="status" class="form-control custom-select mt-15 @error('status') is-invalid @enderror" name="status">
                                            <option value="1" @if($design->status == 1) selected @endif>Active</option>
                                            <option value="0" @if($design->status == 0) selected @endif>In-Active</option>
                                        </select>
                                ...
                                <div class="gb-body">
                                    <div class="form-group">
                                        ...
                                        <div id="dynamic-video-cover">
                                            @if($design->videos) @foreach($design->videos as $video)
                                            <div class="row dynamic-added-video">
                                                <div class="col-md-10">
                                                <textarea name="video[]" class="form-control" rows="1">{{$video->name}}</textarea>
                                                </div>
                                                <div class="col-md-2">
                                                    <a href="/admin/product/design/shirt/video/delete/{{$video->id}}" class="btn btn-danger remove_video">X</a>
                                                </div>
                                            </div>
                                            @endforeach @endif
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div id="p_image_preview" class="row">@foreach($design->images as $image) @if($image->position_id == 1)<div class='col-md-4 upload-multi-img'><img src="/images/product/design/{{$image->name}}" alt="{{$image->name}}"></div>@endif @endforeach</div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div id="s_image_preview" class="row">@foreach($design->images as $image) @if($image->position_id == 2)<div class='col-md-4 upload-multi-img'><img src="/images/product/design/{{$image->name}}" alt="{{$image->name}}"></div>@endif @endforeach</div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div id="images_preview" class="row">
                                            @foreach($design->images as $image) @if($image->position_id == 3)
                                                <div class='col-md-4 upload-multi-img'>
                                                    <img src="/images/product/design/{{$image->name}}" alt="{{$image->name}}">
                                                    <a href="/admin/product/design/shirt/image/delete/{{$image->id}}" class="btn btn-md btn-danger btn-listing ">Delete Image</a>
                                                </div>
                                            @endif @endforeach
                                        </div>
                                    </div>
                            ...
                            <div class="tab-pane" id="seoPane">
                                ...
                                <div class="gb-body">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="pageTitle" class="form-control @error('pageTitle') is-invalid @enderror" id="pageTitle" maxlength="255" value="{{ old('pageTitle', $design->pageTitle) }}">
                                        @error('pageTitle') <p class="error-p">{{$errors->first('pageTitle')}}</p> @enderror
                                    </div>
                                    <div class="form-group">
                                        <label for="metatag">Meta Tags</label>
                                        <textarea name="metatag" id="metatag" class="form-control @error('metatag') is-invalid @enderror" rows="5">{{ old('metatag', $design->metatag) }}</textarea>
                                        @error('metatag') <p class="error-p">{{$errors->first('metatag')}}</p> @enderror
                                    </div>
                                    <div class="form-group">
                                        <label for="metadescp">Meta Description</label>
                                        <textarea name="metadescp" id="metadescp" class="form-control @error('metadescp') is-invalid @enderror" rows="5">{{ old('metadescp', $design->metadescp) }}</textarea>
...
@section('footer')
    ...
    <script type="text/javascript">
        var design_id  = {!! json_encode($design->id) !!};
    </script>
