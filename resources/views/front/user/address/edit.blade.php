                                    ...
                                    <div class="" id="px-dashboard">
                                        ...
                                        <div class="px-customer-dasboard-body">
                                            ...
                                            <div class="px-customer-dashboard-form-cover">
                                                <form action="{{ route('front.user.address.update', $address->id) }}" method="POST" enctype="multipart/form-data">
                                                {{ csrf_field() }} {{ method_field('PUT') }}
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <div class="form-group">                                                   
                                                                <label for="first_name">Full Name *</label>
                                                                <input type="text" class="form-control @error('first_name') is-invalid @enderror" value="{{ old('first_name', $address->first_name) }}" id="first_name" name="first_name" placeholder="Please enter first name...">
                                                                @error('first_name') <p class="error-p">{{ $errors->first('first_name') }}</p> @enderror
                                                            </div>
                                                            <div class="form-group">                                                   
                                                                <label for="last_name">Last Name *</label>
                                                                <input type="text" class="form-control @error('last_name') is-invalid @enderror" value="{{ old('last_name', $address->last_name) }}" id="last_name" name="last_name" placeholder="Please enter last name...">
                                                                @error('last_name') <p class="error-p">{{ $errors->first('last_name') }}</p> @enderror
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="email">Email address *</label>
                                                                <input type="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $address->email) }}" id="emai1" name="email" placeholder="Please enter an email...">
                                                                ...
                                                                <div class="row">
                                                                    <div class="col-md-4">
                                                                        <select class="form-control @error('phone') is-invalid @enderror" id="phoneCode" name="phoneCode">
                                                                            ...
                                                                            <option value="{{$phoneCode->id}}" @if($phoneCode->id == $address->phone_code_id) selected @endif>{{$phoneCode->code}}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </div>
                                                                    <div class="col-md-8">
                                                                        <input type="text" class="form-control @error('phone') is-invalid @enderror" value="{{ old('phone', $address->phone) }}" id="phone" name="phone" placeholder="Please enter a phone number...">
                                                                    </div>
                                                                </div>
                                                        ...
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                ...
                                                                <select class="form-control @error('country') is-invalid @enderror" id="country" name="country">
                                                                    ...
                                                                    @foreach($countries as $country)
                                                                        <option value="{{$country->id}}" @if($country->id == $address->zone->country->id) selected @endif>{{$country->name}}</option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                <textarea name="address" id="address" rows="2" class="form-control @error('address') is-invalid @enderror" rows="5">{{ old('address', $address->address) }}</textarea>
                                                                @error('address') <p class="error-p">{{$errors->first('address')}}</p> @enderror
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="city">City *</label>
                                                                <input type="text" class="form-control @error('city') is-invalid @enderror" value="{{ old('city', $address->city) }}" id="city" name="city" placeholder="Please enter the city...">
                                                                @error('city') <p class="error-p">{{ $errors->first('city') }}</p> @enderror
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="postcode">Post Code *</label>
                                                                <input type="text" class="form-control @error('postcode') is-invalid @enderror" value="{{ old('postcode', $address->postcode) }}" id="postcode" name="postcode" placeholder="Please enter the post code...">
...
@section('footer')
    <script type="text/javascript">
        var country_id  = {!!json_encode($address->zone->country->id)!!};
        var zone_id  = {!!json_encode($address->zone_id)!!};
    </script>
